...
for ($i = 0; $i < count($selection); $i++){
  ...
  if (($Month - 3) > 0) { #jos on t ys kvartaali
    ...
    if($Month >= 3) {
        if (($Month - 6) > 0) { #jos on t ys kvartaali
    ...
    if($Month >= 6) {
        if (($Month - 9) > 0) { #jos on t ys kvartaali
...
$data = array(
    "Tuulivoima" => array(
    #tuulivoima kvartaalit jarjestyksessa  
    0 => $alldata[0], 
    1 => $alldata[1], 
    2 => $alldata[2], 
    3 => $alldata[3],
 ...
 file_put_contents('json/FingridData/tuontato.json',  json_encode(array($data))); 
